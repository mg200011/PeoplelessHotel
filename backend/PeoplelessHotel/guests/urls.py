...
urlpatterns += [url(r'^add_person_to_group/(?P<person_group_id>[-\w+\d+]+)/(?P<guest_id>[0-9]+)/$', GuestsService.add_person_to_group),]
urlpatterns += [url(r'^train_person_group/(?P<person_group_id>[-\w+\d+]+)/$', GuestsService.train_person_group),]
urlpatterns += [url(r'^identify_in_person_group/(?P<room_id>[0-9]+)/$', GuestsService.identify_in_person_group),]
urlpatterns += [url(r'^delete_person_group/(?P<person_group_id>[-\w+\d+]+)/$', GuestsService.delete_person_group),]